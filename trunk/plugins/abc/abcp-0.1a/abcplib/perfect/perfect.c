...
/* RD 04 Oct 04 Generate functions with different names */
static char *fname;
...
/* make the .c file */
static void make_c(tab, smax, blen, scramble, final, form,nkeys)
...
#else
  fprintf(f, "typedef  unsigned long long  ub8;\n");
  fprintf(f, "typedef  unsigned long  int  ub4;\n");
  fprintf(f, "typedef  unsigned short int  ub2;\n");
  fprintf(f, "typedef  unsigned       char ub1;\n\n");

  fprintf(f, "#define PHASH %s\n",fname);
  fprintf(f, "#define PHASHNKEYS %ld  /* How many keys were hashed */\n\n", nkeys);

  fprintf(f, "ub4  lookup( ub1 *k, ub4 length, ub4 level );\n");
  fprintf(f, "void checksum( ub1 *k, ub4 length, ub4 *state);\n");
  fprintf(f, "ub2 shash(ub1 *str);\n\n");
  
#endif

  fprintf(f, "\n");
  
  fprintf(f, "#ifndef STATIC\n");
  fprintf(f, "#define STATIC\n");
  fprintf(f, "#endif\n");
...
"0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx,\n",
  ...
  {
  case NORMAL_HM:
    fprintf(f, "STATIC ub4 %s(key, len)\n",fname);
    fprintf(f, "char *key;\n");
    fprintf(f, "int   len;\n");
    break;
  case INLINE_HM:
  case HEX_HM:
  case DECIMAL_HM:
    fprintf(f, "STATIC ub4 %s(val)\n",fname);
    fprintf(f, "ub4 val;\n");
    break;
  case AB_HM:
  case ABDEC_HM:
    fprintf(f, "STATIC ub4 %s(a,b)\n",fname);
    fprintf(f, "ub4 a;\n");
    fprintf(f, "ub4 b;\n");
    break;
  }
...
hashform *form;                                           /* user directives */
{
  ...
  make_c(tab, smax, blen, scramble, &final, form, nkeys);
...
/* RD 04 Oct 04 Added "fname" */
static void usage_error()
{
  printf("Usage: perfect [-{NnIiHhDdAaBb}{MmPp}{FfSs} [fname]] < key.txt \n");
